...
name = "2"
image = cv2.imread('D:/temp/{}.jpg'.format(name))
...
def click_event(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        if mode == 1:
            ...
            dot_index = (len(points_group1) - 1) % len(dots)
            cv2.circle(pose_map, (x, y), 5, dots[dot_index], -1)

            if len(points_group1) > 1:
                line_index = (len(points_group1) - 2) % len(lines)
                cv2.line(pose_map, points_group1[-2], points_group1[-1], lines[line_index], 2)
        elif mode == 2:
            points_group2.append((x, y))
            dot_index = (len(points_group2) - 1) % len(dots2)
            cv2.circle(pose_map, (x, y), 5, dots2[dot_index], -1)

            if len(points_group2) > 1:
                line_index = (len(points_group2) - 2) % len(lines2)
                cv2.line(pose_map, points_group2[-2], points_group2[-1], lines2[line_index], 2)
